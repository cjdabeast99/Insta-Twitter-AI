...
def generate_interesting_fact_with_gpt():
    ...
    prompt = f"Generate a {selected_genre} {selected_info} that is intriguing about {selected_topic}. (Return only the fact. Limit 50 characters)"
...
def generate_meme_caption_with_gpt(fact):
    ...
    selected_actor = random.choice(actor)
    prompt = f"You are {selected_actor}, write a funny meme-like caption for this fact: '{fact}' (DO NOT repeat the fact. Limit 99 characters) Include a lot of emojis and relevant hashtags"

    # Introduce a delay of 5 seconds between API calls
    rate_limit_delay(5)

    response = execute_with_retry(openai.Completion.create, engine="text-davinci-002", prompt=prompt, max_tokens=100)
...
def search_related_image(query):
    ...
    search_query = f"{query} liminal space"
...
# image_url = search_related_image(fact)
image_url = search_unsplash_image(fact)
time.sleep(2)
...
if image_url:
    ...
    if response.status_code == 200:
        ...
        # Upload the image with the meme-like caption
        bot.photo_upload(image_path, caption)
